...
class BdProfiler{
    ...
    static public function start(){
        ...
        $arrInfo = array('begTime' => $beg, 'module' => Vega\Init\AppEnv::getCurrApp());
    ...
    static public function dumpProfInfo(){
        ...
        foreach($arrLogInfo as $module => $logStr){
            $fileName = Vega\Init\AppEnv::getEnv('log', $module).'/'.$module.'.profiler';
    ...
    static private function _enableProfiling(){
        if(!self::$bolInit){
            $arrConf = Vega\Conf\Log::getConf('profiler/log');
            if(false !== $arrConf){
    ...
    static private function _getLogString($current_args){
        ...
        $str .=	' [logid=' . (defined('LOG_ID')? LOG_ID : Vege\Init\Env::getLogID());
        $args = array();
